...
export const NavBar = ({ theme, toggleTheme }: NavBarProps) => (
    <nav className="fixed top-0 left-0 w-full p-4 md:p-6 flex justify-between items-center z-50 text-[var(--text-main)] transition-colors duration-500">
        ...
        <div className="flex items-center gap-2 md:gap-4">
            ...
            <button className="hidden md:block px-5 py-2 border border-[var(--border-color)] rounded-full text-xs uppercase tracking-widest hover:bg-[var(--text-main)] hover:text-[var(--bg-main)] transition-all duration-300">
                Sign In
            </button>
        </div>
    </nav>
...
export const MobileNav = () => {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 md:hidden z-50 bg-[var(--bg-main)]/90 backdrop-blur-lg border-t border-[var(--border-color)]">
